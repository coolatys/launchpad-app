...
import { Compass, Mail, Lock, User, ArrowRight, ShieldCheck, Loader2, CheckCircle2, AlertCircle, KeyRound } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (tab === 'signin') {
      ...
      if (error) {
        ...
        setStatus({
          type: 'success',
          text: message || 'Account created! Check your email for verification link.',
        });
        setEmail('');
        setPassword('');
        setName('');
      ...
      <div className="max-w-md w-full space-y-6 bg-white p-8 rounded-3xl border border-slate-200 shadow-xl">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-slate-500 max-w-xs mx-auto">
            {tab === 'signin' && 'Sign in to access your personal candidate matching pipeline.'}
            {tab === 'signup' && 'Create your account to start automated career matching.'}
            {tab === 'reset' && 'Reset your account password.'}
          </p>
        </div>
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200">
          <button
            onClick={() => { setTab('signin'); setStatus(null); }}
            className={`flex-1 py-2 text-xs font-bold rounded-xl transition ${
              tab === 'signin' ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            Sign In
          </button>
          <button
            onClick={() => { setTab('signup'); setStatus(null); }}
            className={`flex-1 py-2 text-xs font-bold rounded-xl transition ${
              tab === 'signup' ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            Sign Up
          </button>
          <button
            onClick={() => { setTab('reset'); setStatus(null); }}
            className={`flex-1 py-2 text-xs font-bold rounded-xl transition ${
              tab === 'reset' ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            Reset
          </button>
        ...
        {tab !== 'reset' && (
          <>
            <button
              ...
              className="w-full flex items-center justify-center gap-3 px-5 py-3 border border-slate-300 rounded-2xl font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-400 transition shadow-sm cursor-pointer"
            >
            ...
            <div className="relative flex items-center justify-center my-3">
              ...
              <span className="bg-white px-3 text-xs text-slate-400 font-medium uppercase absolute">Or Email & Password</span>
            </div>
          </>
        )}
          ...
          >
            {loading ? (
              ...
              <>
                ...
                {tab === 'reset' && 'Send Password Reset Email'}
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-center gap-2 text-xs text-slate-400">
          ...
          <span>Encrypted Supabase Security & Data Privacy</span>
        </div>
      </div>
    </div>
  );
